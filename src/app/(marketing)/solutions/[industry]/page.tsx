        ...
        <section className="bg-white border-2 border-slate-300 rounded-[32px] sm:rounded-[44px] p-8 sm:p-14 lg:p-20 shadow-sm mb-10" id="industry-products">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="industry-products-grid">
            {industry.products.map((product, index) => (
              <ScrollReveal key={product.slug} delay={index * 0.08} yOffset={24} duration={0.6}>
                <Tilt3D maxTilt={4} scale={1.01} className="h-full">
                  <div
                    className="h-full p-8 bg-[#fafafa] border-2 border-slate-200 hover:border-sky-500 rounded-3xl transition-all duration-300 flex flex-col justify-between hover:shadow-lg relative"
