  ...
  return (
    <div className="bg-[#f7f7f5] text-[#0d0d0e] min-h-screen pb-16 pt-28 sm:pt-36 px-3 sm:px-6 lg:px-8 selection:bg-[#0d0d0e] selection:text-white">
      <main className="max-w-[1536px] w-full mx-auto" id="careers-main">
        ...
        {/* Page Header */}
        <section className="bg-white border-2 border-slate-300 rounded-[32px] sm:rounded-[44px] p-8 sm:p-14 lg:p-20 shadow-sm mb-10 text-center relative overflow-hidden grid-pattern-light" id="careers-hero">
        ...
        {/* Open Positions List */}
        <section className="bg-white border-2 border-slate-300 rounded-[32px] sm:rounded-[44px] p-8 sm:p-14 lg:p-20 shadow-sm mb-10" id="open-positions-section">
          ...
          {openPositions.length === 0 ? (
            <div className="p-8 sm:p-12 bg-[#fafafa] border-2 border-slate-200 rounded-3xl text-center">
            ...
            <div className="space-y-4">
              {openPositions.map((pos) => (
                <div key={pos.slug} className="p-6 bg-[#fafafa] border-2 border-slate-200 hover:border-sky-500 rounded-3xl flex items-center justify-between transition-all">
                  <div>
                    <h3 className="text-lg font-bold text-[#0d0d0e]">{pos.title}</h3>
                    <p className="text-xs text-slate-500">{pos.department} • {pos.location}</p>
                  </div>
                  <Link href={`/careers/${pos.slug}`} className="px-5 py-2 rounded-full bg-[#0d0d0e] text-white text-xs font-bold hover:bg-sky-600 transition-colors">
                    Apply Now
                  </Link>
                </div>
              ))}
        ...
        <section className="bg-[#0d0d0e] text-white border-2 border-slate-700 rounded-[32px] sm:rounded-[44px] p-8 sm:p-14 lg:p-16 text-center shadow-2xl relative overflow-hidden" id="careers-cta">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-sm sm:text-base text-slate-300 mb-8 max-w-xl mx-auto">
              Send your resume and portfolio directly to our engineering lead at <strong className="text-sky-400">hello@ohotech.com</strong>.
            </p>
            <Link
