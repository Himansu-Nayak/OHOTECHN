                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-3 pt-2 sm:pt-0 border-t sm:border-0 border-slate-100">
                    ...
                    {(sub.status === 'ACTIVE' || sub.status === 'TRIAL') && (
                      <button
                        ...
                        className="min-h-[44px] py-2.5 px-4 rounded-full bg-rose-50 hover:bg-rose-100 text-rose-700 font-mono font-bold text-xs border border-rose-200 transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                      >
                        <XCircle className="w-4 h-4" />
                        <span>{cancellingId === sub.id ? 'Cancelling...' : 'Cancel Subscription'}</span>
                      </button>
                    )}
                  </div>
                </div>
