...
export function ERPStorySection() {
  const capabilities = [
    { title: 'ERP Solutions', desc: 'Centralized enterprise platforms unifying multi-department business processes.', emoji: '⚙️' },
    { title: 'CRM Systems', desc: 'Lead tracking, customer communications, sales pipelines, and support tickets.', emoji: '👥' },
    { title: 'HRMS & Payroll', desc: 'Employee records, attendance tracking, automated payroll, and compliance.', emoji: '💼' },
    { title: 'Inventory Management', desc: 'Real-time stock level tracking, warehouse orders, and supplier automation.', emoji: '📦' },
    { title: 'Business Automation', desc: 'Streamlining approval chains and manual cross-department tasks.', emoji: '🔄' },
  ];

  return (
    <section className="max-w-[1536px] w-full mx-auto mb-6 bg-[#fafafa] border-2 border-slate-300 rounded-[32px] sm:rounded-[44px] p-8 sm:p-14 lg:p-20 shadow-sm relative overflow-hidden transition-all duration-500" id="erp-story">
      ...
      <ScrollReveal yOffset={15} duration={0.6}>
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 font-mono text-xs font-bold uppercase tracking-wider mb-5">
          <span>⚙️</span>
          <span>04 / ENTERPRISE ERP &amp; BUSINESS SYSTEMS</span>
        </div>
      </ScrollReveal>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-14">
        ...
        <div className="lg:col-span-5 order-2 lg:order-1 flex justify-center">
          <Tilt3D maxTilt={5} scale={1.02} className="w-full max-w-md">
            <div className="w-full bg-white border border-slate-200 rounded-3xl p-8 shadow-xl relative overflow-hidden group hover:border-emerald-400 transition-all duration-300">
        ...
        <ScrollReveal yOffset={25} duration={0.7} className="lg:col-span-7 order-1 lg:order-2">
          <h2 className="text-3xl sm:text-5xl lg:text-6xl font-black text-[#0d0d0e] tracking-tight leading-[1.08] mb-6">
            Bring your business into one connected system. ⚙️
          </h2>
          ...
          {capabilities.map((item, idx) => {
            return (
              <ScrollReveal key={idx} delay={idx * 0.08} yOffset={20} duration={0.6}>
                <div className="h-full p-5 rounded-2xl bg-white border-2 border-slate-200 hover:border-emerald-400 transition-all group">
