    ...
    <section className="max-w-[1536px] w-full mx-auto mb-16 bg-white border-2 border-slate-300 rounded-[32px] sm:rounded-[44px] p-8 sm:p-14 lg:p-20 shadow-sm relative overflow-hidden" id="how-we-work">
      ...
      <div className="max-w-4xl mb-16 text-left">
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 font-mono text-xs font-bold uppercase tracking-wider mb-4">
          ...
          07 / DELIVERY METHODOLOGY ROADMAP ⚡
        </div>
        ...
        <div className="flex items-center justify-between -mt-5 relative z-10 px-4">
          {steps.map((s, idx) => (
            <div key={s.num} className="flex items-center gap-2">
              ...
              {idx < steps.length - 1 && (
                <div className="hidden xl:flex items-center text-emerald-600 text-xs font-mono font-bold">
                  <span>➔</span>
                </div>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 relative z-10">
        {steps.map((step, idx) => {
          return (
            <div
              key={step.num}
              className="p-6 sm:p-7 rounded-3xl bg-[#fafafa] border-2 border-slate-200 hover:border-emerald-500 transition-all duration-300 flex flex-col justify-between group hover:shadow-xl relative overflow-hidden"
            >
              ...
              <div>
                ...
                <div className="flex items-center justify-between mb-5 pb-3 border-b border-slate-200/80">
                  <div className="flex items-center gap-2">
                    <span className="w-8 h-8 rounded-xl bg-[#0d0d0e] text-white flex items-center justify-center font-mono text-xs font-black shadow-md">
                      {step.num}
                    </span>
                ...
                {/* Step Title & Subtitle */}
                <h3 className="text-lg font-extrabold text-[#0d0d0e] mb-1 flex items-center gap-1.5 group-hover:text-emerald-600 transition-colors">
                  <span>{step.emoji}</span>
                  <span>{step.title}</span>
                </h3>

                <div className="text-[11px] font-mono text-emerald-700 font-semibold mb-3">
                  {step.subtitle}
                </div>
              ...
              <div className="pt-4 border-t border-slate-200/80 flex items-center justify-between text-[11px] font-mono text-slate-700">
      ...
      <div className="mt-12 bg-[#0d0d0e] text-white rounded-3xl p-6 sm:p-8 border-2 border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
